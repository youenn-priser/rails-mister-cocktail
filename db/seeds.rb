puts 'Deleting cocktails, doses and ingredients....'
Cocktail.destroy_all
Dose.destroy_all
Ingredient.destroy_all
print 'OK'

puts 'Creating cocktails, doses and ingredients....'
...
recipes.each do |recipe|
  ...
  recipe[:ingredients].each do |item|
    if item[:label].nil?
      if Ingredient.where(name: item[:ingredient]).take.nil?
        ingredient = Ingredient.create!(name: item[:ingredient])
      else
        ingredient = Ingredient.where(name: item[:ingredient]).take
      end
    else
      if Ingredient.where(name: item[:label]).take.nil?
        ingredient = Ingredient.create!(name: item[:label])
      else
        ingredient = Ingredient.where(name: item[:label]).take
      end
    end
    p ingredient
    dose = Dose.create!(
